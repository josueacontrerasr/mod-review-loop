...
## Auditoría del ZIP `diable_shaders_78cc3.zip`

El archivo adjunto tiene SHA-256 `f33ac62838eee9a74b19b27b6703071d98e24b68067026eb7608601ea323a09e`, 22 entradas y una sola raíz `optimods/`. Contiene `_polymod_meta.json`, tres JSON de stages y 12 scripts HScript `.hxc`; no contiene APK, DLL, ejecutables, shell scripts, rutas absolutas ni traversal evidente. Su manifiesto declara `title: Optimod`, `mod_version: 1.0.0`, licencia Apache-2.0 y `api_version: 0.7.0`.

El código fuente oficial FNF v0.8.6 fija `PolymodHandler.API_VERSION_RULE` en `>=0.8.0 <0.9.0`, por lo que el manifiesto original con `0.7.0` no puede cargarse directamente como mod compatible con 0.8.6. El mismo código proporciona alias para `funkin.graphics.adobeanimate.FlxAtlasSprite` y `funkin.modding.base.ScriptedFlxAtlasSprite`, y el enum `WiggleEffectType` está definido dentro del módulo `WiggleEffectRuntime`; esos imports legacy no son por sí solos un fallo. En cambio, `funkin.play.character.CharacterType` debe compararse con `funkin.play.character.BaseCharacter.CharacterType` de v0.8.6, porque el enum moderno está anidado en `BaseCharacter` y no aparece un alias directo en `PolymodHandler`.

El optimizador usa callbacks de stage/song, shaders, `FlxTrail`, `FlxFlicker`, cámaras adicionales, vídeo/cutscene de `Stress` y acceso a `Save`. La auditoría no encontró `Sys`, `haxe.Http`, `NativeProcess`, `cpp.Lib`, `lime.system.CFFI/JNI` ni APIs de ejecución de procesos. Por seguridad y compatibilidad, debe conservarse el ZIP original intacto y crear una copia de laboratorio adaptada a 0.8.6 con un ID separado, manifest actualizado y cambios mínimos de imports; nunca debe mezclarse automáticamente dentro de los ZIPs Esperón.

Fuente primaria de estas reglas: https://github.com/FunkinCrew/Funkin/blob/v0.8.6/source/funkin/modding/PolymodHandler.hx
Fuente primaria del enum: https://github.com/FunkinCrew/Funkin/blob/v0.8.6/source/funkin/play/character/BaseCharacter.hx
Fuente primaria del módulo de shaders: https://github.com/FunkinCrew/Funkin/blob/v0.8.6/source/funkin/graphics/shaders/WiggleEffectRuntime.hx
