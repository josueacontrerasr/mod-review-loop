...
OUT_ROOT = ROOT / "qa-lab" / "rebuild-v263" / "playstate-fix" / "syllable-candidates"
...
def make_syllables(words: list[dict[str, Any]], env_t: np.ndarray, env_r: np.ndarray) -> list[dict[str, Any]]:
    ...
    for i, item in enumerate(syllables):
        ...
        vocal_end = min(float(item["word_end_ms"]), float(following) - 35.0)
        ...
        item["hold_ms"] = round(min(900.0, duration - 35.0), 3) if duration >= 420.0 else 0.0
        if item["hold_ms"] > 0:
...
def note_from(item: dict[str, Any], timestamp: float, lane: int) -> dict[str, Any]:
    note: dict[str, Any] = {"t": round(float(timestamp), 3), "d": int(lane % 4)}
...
def difficulty_notes(syllables: list[dict[str, Any]], difficulty: str) -> list[dict[str, Any]]:
    if difficulty == "easy":
        selected = [s for i, s in enumerate(syllables) if i % 2 == 0 or s["kind"].startswith("interjection") or s.get("hold_ms", 0) >= 420]
    else:
        selected = syllables
    notes=[]
    used={}
...
def process_one(model: Any, song: dict[str, str]) -> dict[str, Any]:
    ...
    report = {
        "scope": "VOCAL_SYLLABLE_ALIGNED_CANDIDATE",
        ...
        "policy": [
            ...
            "Easy reduces density from the same timestamps; normal and hard preserve all reliable syllables.",
...
def main() -> int:
    ...
    if args.workers > 1:
        ...
        for song in songs:
            results.append(process_one(model, song))
    summary = {"scope": "V263_SYLLABLE_ALIGNMENT_BATCH", "status": "MANUAL_REVIEW_REQUIRED", "model": args.model, "songs": len(results), "results": results}
