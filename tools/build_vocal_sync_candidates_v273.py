...
ALIGN_ROOT = ROOT / "qa-lab/rebuild-v272/playstate-fix/vocal-sync-candidates-canonical"
OUT_ROOT = ROOT / "qa-lab/rebuild-v273/playstate-fix/vocal-sync-candidates"
SONGS = [
...
def attack_start_ms(item: dict[str, Any]) -> float:
    return float(item.get("audio_onset_ms", item.get("start_ms", 0.0)))


def measured_hold(item: dict[str, Any], next_start: float) -> float:
    kind = str(item.get("kind", ""))
    measured = float(item.get("hold_ms", 0.0) or 0.0)
    start = attack_start_ms(item)
...
def make_note(item: dict[str, Any], direction: int, next_start: float, subdivision: bool = False) -> dict[str, Any]:
    start = round(attack_start_ms(item), 3)
...
def build_base_notes(syllables: list[dict[str, Any]], directions: list[int], difficulty: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for index, item, subdivision in selected:
        next_start = attack_start_ms(syllables[index + 1]) if index + 1 < len(syllables) else 10**12
        ...
        if collision is not None:
            # Preserve a distinct syllable whenever another player lane is free.
            alternatives = [(int(note["d"]) + step) % 4 for step in (1, 2, 3)]
            replacement = next(
                (lane for lane in alternatives if not any(old_d == lane and 0.0 <= float(note["t"]) - old_t < ENGINE_COLLISION_MS for old_t, old_d in occupied)),
                None,
            )
            if replacement is None:
                report.append({"reason": "sub_12ms_same_lane_collision_deduplicated", "t": note["t"], "d": note["d"], "kept_t": collision[0]})
                continue
            report.append({"reason": "sub_12ms_same_lane_collision_reassigned_free_player_lane", "t": note["t"], "from_d": note["d"], "to_d": replacement, "kept_t": collision[0]})
            note["d"] = replacement
...
def apply_density_policy(notes: list[dict[str, Any]], difficulty: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    # V2.7.3 deliberately preserves all measured vocal attacks. The prior 500 ms
...
def process_song(song: str) -> dict[str, Any]:
    ...
    syllables = sorted((dict(item) for item in alignment.get("syllables", [])), key=attack_start_ms)
    ...
    candidate_alignment["version"] = "V2.7.3"
    candidate_alignment["syllables"] = syllables
    candidate_alignment["chart_policy"] = "one vocal attack per aligned syllable at audio_onset_ms; no 500ms or 1000ms density deletion; measured holds only for sustained_syllable/interjection_hold"
    ...
    report = {
        "scope": "V273_CALIBRATED_VOCAL_ONSET_CANDIDATE",
...
def main() -> int:
    ...
    summary = {
        "scope": "WIDE_RESEARCH_V273_CALIBRATED_VOCAL_ONSET_CANDIDATES",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "base_release": "esperon-vslice-086-v2.7.2",
    ...
    output = OUT_ROOT / "batch-summary-v273.json"
