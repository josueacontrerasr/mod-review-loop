...
def one(song: str) -> dict[str,Any]:
    ...
    align_path=ROOT/'qa-lab/rebuild-v265/playstate-fix/syllable-candidates-small'/song/'syllable-alignment.json'
    ...
    errors=[]; counts={}; outside=0; leaked=0; bad_lanes=0; bad_holds=0; unaligned=0
    for diff in DIFFS:
        notes=chart.get('notes',{}).get(diff,[]); counts[diff]=len(notes)
        if {int(n.get('d',-1)) for n in notes} != {4,5,6,7}: errors.append(f'lane_coverage_not_four_player_directions:{diff}')
        for idx,n in enumerate(notes):
            if set(n)-{'t','d','l','k','p'}: leaked+=1
            if not isinstance(n.get('d'),int) or not 4<=n['d']<=7: bad_lanes+=1
    ...
    if chart.get('generatedBy') != "Friday Night Funkin' - 0.8.6; V2.6.5 syllable-aligned vocal chart player lanes cycling 4..7": errors.append('chart_generatedBy_invalid')
    if chart.get('candidateOnly') is not None or chart.get('sourcePolicy') is not None: errors.append('candidate_fields_leaked')
    return {'song':song,'status':'PASS' if not errors else 'ERRORS_FOUND','counts':counts,'outside_syllable_intervals':outside,'unaligned_notes':unaligned,'bad_holds':bad_holds,'candidate_metadata_leaked':leaked,'bad_player_lanes':bad_lanes,'generatedBy':chart.get('generatedBy'),'errors':errors}

def main():
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as pool: rows=sorted(pool.map(one,SONGS),key=lambda x:x['song'])
    payload={'scope':'PRODUCTION_VOCAL_SYLLABLE_GATE_V265','executed_at':datetime.now(timezone.utc).isoformat(),'target_version':'0.8.6','mod_version':'2.6.5','songs':len(rows),'passed':sum(x['status']=='PASS' for x in rows),'status':'PASS' if all(x['status']=='PASS' for x in rows) else 'ERRORS_FOUND','total_notes_outside_syllable_intervals':sum(x['outside_syllable_intervals'] for x in rows),'total_unaligned_notes':sum(x['unaligned_notes'] for x in rows),'total_bad_holds':sum(x['bad_holds'] for x in rows),'rows':rows}
    out=ROOT/'qa-lab/rebuild-v265/playstate-fix/production-syllable-gate-v265.json'; out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n')
