...
def validate_package(path: Path) -> dict:
    ...
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            song_dirs = sorted({name.rsplit("/", 1)[0] for name in names if name.startswith(prefix + "data/songs/") and name.count("/") >= 4})
            if len(song_dirs) != 1:
            ...
            if meta_name in names and chart_name in names:
                ...
                for difficulty in ("easy", "normal", "hard"):
                    ...
                    if any(t < 0 or d < 0 or d > 3 for t, d in keys): errors.append(f"domain:{difficulty}")
            ...
            title_candidates = [name for name in names if name.startswith(prefix + "images/freeplay/albumRoll/") and name.endswith("-title.xml")]
...
def main() -> int:
    ...
    if not collection.is_file(): collection_errors.append("collection_missing")
        ...
        with zipfile.ZipFile(collection) as archive:
            ...
            if "README-INSTALACION.txt" not in names: collection_errors.append("collection_readme_missing")
            for package in packages:
                if package.name not in names: collection_errors.append(f"collection_missing:{package.name}")
            allowed = {"README-INSTALACION.txt"} | {package.name for package in packages}
            extras = sorted(name for name in names if name not in allowed and not name.endswith("/"))
            if extras: collection_errors.append(f"collection_unexpected:{extras}")
    ...
    output = ROOT / "qa-lab" / "rebuild-v250" / "zip-validation-v250.json"
