...
def main():
    ...
    manifest={'version':'2.4.0','status':'PASS','mods':len(paths),'packages':[{'file':p.name,'bytes':p.stat().st_size,'sha256':sha(p),'root_entries':sorted({x.split('/')[0] for x in zipfile.ZipFile(p).namelist()})} for p in paths]}
    manifest_path=ROOT/'qa-lab/rebuild-v240/package-manifest-v240.json'; manifest_path.write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
