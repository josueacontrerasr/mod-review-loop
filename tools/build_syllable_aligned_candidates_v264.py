...
OUT_ROOT = ROOT / "qa-lab" / "rebuild-v264" / "playstate-fix" / "syllable-candidates"
...
def difficulty_notes(syllables: list[dict[str, Any]], difficulty: str) -> list[dict[str, Any]]:
    ...
    for index,item in enumerate(selected):
        ...
        # Cycle through the four player directions in chronological order;
        # use count as a collision offset for simultaneous notes.
        lane=count % 4
...
def process_one(model: Any, song: dict[str, str]) -> dict[str, Any]:
    ...
    report = {
        "scope": "VOCAL_SYLLABLE_ALIGNED_PLAYER_LANE_CANDIDATE_V264",
...
def main() -> int:
    ...
    if args.workers > 1:
        ...
        for song in songs:
            results.append(process_one(model, song))
    summary = {"scope": "V264_SYLLABLE_ALIGNMENT_BATCH", "status": "MANUAL_REVIEW_REQUIRED", "model": args.model, "songs": len(results), "results": results}
