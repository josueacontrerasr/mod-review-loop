...
def main() -> int:
    ...
    output = root / "qa-lab" / "rebuild-v273" / "native-mobile-smoke-v273"
    if output.exists():
        shutil.rmtree(output)
    output.mkdir(parents=True, exist_ok=True)
    source_root = root / "mods"
    persistent_mod = root / "qa-lab" / "rebuild-v273" / "persistent-mods" / "normalized" / "optimods"
    if not persistent_mod.is_dir():
        raise SystemExit(f"ERROR: falta el optimizador persistente normalizado: {persistent_mod}")

    report: dict[str, Any] = {
        "scope": "NATIVE_ANDROID_SMOKE_V273",
