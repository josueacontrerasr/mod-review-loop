...
def one(root: Path, song: str, round_no: int) -> dict:
    ...
    try:
        ...
        if poly.get("api_version") != "0.8.6" or poly.get("mod_version") != "2.6.5": issues.append("manifest_contract")
        ...
        if chart.get("generatedBy") != "Friday Night Funkin' - 0.8.6; V2.6.5 syllable-aligned vocal chart player lanes cycling 4..7": issues.append("chart_generatedBy")
        ...
        for diff, notes in chart.get("notes", {}).items():
            ...
            if any(t < 0 or d < 4 or d > 7 for t, d in keys): issues.append(f"chart_{diff}_lane_domain")
            if {d for _, d in keys} != {4, 5, 6, 7}: issues.append(f"chart_{diff}_lane_coverage")
    ...
    archive_name = f"Mod-{'-'.join(word.capitalize() for word in song.split('-'))}-V2.6.5.zip"; archive = root / "Mods .zip terminados" / archive_name
    try:
...
def main() -> int:
    ...
    payload = {"scope": "QA_20X21_V265", "status": "PASS" if all(item["status"] == "PASS" for item in rounds) and complete_status == "PASS" else "ERRORS_FOUND", "version": "2.6.5", "rounds": 20, "mods_per_round": 21, "total_reviews": 420, "parallel_workers": 8, "complete_zip_crc": complete_status, "rows": rounds}
    output = root / "qa-lab" / "rebuild-v265" / "playstate-fix" / "qa-20x21-v265.json"; output.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"); print(json.dumps({"status": payload["status"], "rounds": 20, "total_reviews": 420, "complete_zip_crc": complete_status, "output": str(output)}, ensure_ascii=False)); return 0 if payload["status"] == "PASS" else 1
