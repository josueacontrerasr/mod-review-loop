    ...
    for difficulty in DIFFICULTIES:
        if difficulty not in notes: errors.append(f"difficulty_missing:{difficulty}")
            ...
            for index, note in enumerate(notes[difficulty]):
                ...
                if "d" in note and (not isinstance(note["d"], int) or note["d"] < 0 or note["d"] > 7): errors.append(f"note_d_type_or_domain:{difficulty}[{index}]")
...
def main() -> int:
    ...
    payload = {
        "scope": "PLAYSTATE_VARIATION_RESOLVER_V262",
    ...
    output = root / "qa-lab" / "rebuild-v262" / "playstate-fix" / "playstate-resolver-production-v262.json"
