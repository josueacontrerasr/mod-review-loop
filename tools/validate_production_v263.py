...
def one(song: str) -> dict[str,Any]:
    ...
    align_path=ROOT/'qa-lab/rebuild-v263/playstate-fix/syllable-candidates'/song/'syllable-alignment.json'
    ...
    if chart.get('generatedBy') != "Friday Night Funkin' - 0.8.6; V2.6.3 syllable-aligned vocal chart": errors.append('chart_generatedBy_invalid')
...
def main():
    ...
    payload={'scope':'PRODUCTION_VOCAL_SYLLABLE_GATE_V263','executed_at':datetime.now(timezone.utc).isoformat(),'target_version':'0.8.6','mod_version':'2.6.3','songs':len(rows),'passed':sum(x['status']=='PASS' for x in rows),'status':'PASS' if all(x['status']=='PASS' for x in rows) else 'ERRORS_FOUND','total_notes_outside_syllable_intervals':sum(x['outside_syllable_intervals'] for x in rows),'total_unaligned_notes':sum(x['unaligned_notes'] for x in rows),'total_bad_holds':sum(x['bad_holds'] for x in rows),'rows':rows}
    out=ROOT/'qa-lab/rebuild-v263/playstate-fix/production-syllable-gate-v263.json'; out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n')
