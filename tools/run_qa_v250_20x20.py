...
def one(root: Path, song: str, round_no: int) -> dict:
    ...
    try:
        ...
        if manifest.get("api_version") != "0.8.6" or manifest.get("mod_version") != "2.5.0": issues.append("manifest_contract")
    ...
    for archive in (root / "Mods .zip terminados").glob("Mod-*-V2.5.0.zip"):
...
def main() -> int:
    ...
    payload = {"status": "PASS" if all(item["status"] == "PASS" for item in rounds) else "ERRORS_FOUND", "version": "2.5.0", "rounds": 20, "mods_per_round": 20, "total_reviews": 400, "parallel_workers": 8, "rows": rounds, "scope": "Each round scans every source file and individual V2.5.0 ZIP CRC; collection is validated separately."}
    output = root / "qa-lab" / "rebuild-v250" / "qa-20x20-v250.json"
