...
SOURCE=ROOT/'qa-lab/rebuild-v263/playstate-fix/syllable-candidates-small'
CANON=ROOT/'qa-lab/rebuild-v263/playstate-fix/syllable-candidates'
BACKUP=ROOT/'qa-lab/rebuild-v263/playstate-fix/production-before-v263'
...
def main():
    ...
    for mod in sorted((ROOT/'mods').glob('esperon-dano-*')):
        ...
        backup=BACKUP/f'{song}-chart-before-v263.json'; backup.parent.mkdir(parents=True,exist_ok=True); shutil.copy2(prod,backup)
        chart=json.loads(cand.read_text())
        chart['generatedBy']="Friday Night Funkin' - 0.8.6; V2.6.3 syllable-aligned vocal chart"
    ...
    report={'scope':'V263_SYLLABLE_CHART_PROMOTION','status':'PASS' if len(rows)==21 else 'FAIL','songs':len(rows),'source':'Whisper small + RMS refinement','rows':rows}
    out=ROOT/'qa-lab/rebuild-v263/playstate-fix/syllable-promotion-v263.json'; out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
