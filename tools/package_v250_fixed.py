...
def package_mod(song):
    ...
    out=DELIVERY/f'Mod-{display(song)}-V2.5.0.zip'
...
def main():
    ...
    manifest={'version':'2.5.0','status':'PASS','mods':len(paths),'packages':[{'file':p.name,'bytes':p.stat().st_size,'sha256':sha(p),'root_entries':sorted({x.split('/')[0] for x in zipfile.ZipFile(p).namelist()})} for p in paths]}
    manifest_path=ROOT/'qa-lab/rebuild-v250/package-manifest-v250.json'; manifest_path.write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    collection=DELIVERY/'Mod-Esperon-Coleccion-V2.5.0.zip'
    readme='Esperón FNF Mobile V-Slice 0.8.6 V2.5.0\n\nContiene 20 ZIPs individuales. Extrae el ZIP individual elegido directamente dentro de la carpeta mods de FNF Mobile.\n'
