...
def one(root: Path, song: str) -> dict[str, Any]:
    ...
    if not polymod.is_file(): errors.append("polymod_missing")
        ...
        if data.get("mod_version") != "2.6.6": errors.append(f"mod_version:{data.get('mod_version')}")
    ...
    if chart.get("generatedBy") != "Friday Night Funkin' - 0.8.6; V2.6.6 vocal syllable vowel-mapped player lanes d=0..3": errors.append("chart_generatedBy")
...
def main() -> int:
    ...
    payload = {"scope": "VSLICE_086_CONTRACTS_ASSETS_V266", "executed_at": datetime.now(timezone.utc).isoformat(), "target_version": "0.8.6", "mod_version": "2.6.6", "songs": len(rows), "passed": sum(row["status"] == "PASS" for row in rows), "status": "PASS" if all(row["status"] == "PASS" for row in rows) else "ERRORS_FOUND", "rows": rows}
    output = root / "qa-lab" / "rebuild-v266" / "playstate-fix" / "contracts-assets-v266.json"; output.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
