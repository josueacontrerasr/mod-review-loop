...
def one(root: Path, song: str, round_no: int) -> dict:
    ...
    try:
        ...
        if poly.get("api_version") != "0.8.6" or poly.get("mod_version") != "2.7.3": issues.append("manifest_contract")
        ...
        if chart.get("generatedBy") != "Friday Night Funkin' - 0.8.6; V2.7.3 calibrated vocal onsets, syllable-accurate holds and player lanes d=0..3": issues.append("chart_generatedBy")
    ...
    archive_name = f"Esperon-{'-'.join(word.capitalize() for word in song.split('-'))}-V2.7.3.zip"; archive = root / "Mods .zip terminados" / archive_name
    try:
...
def main() -> int:
    ...
    complete = root / "Mods .zip terminados" / "Esperon-Completo-V2.7.3.zip"
    ...
    payload = {"scope": "QA_20X21_V273", "status": "PASS" if all(item["status"] == "PASS" for item in rounds) and complete_status == "PASS" else "ERRORS_FOUND", "version": "2.7.3", "rounds": 20, "mods_per_round": 21, "total_reviews": 420, "parallel_workers": 8, "complete_zip_crc": complete_status, "rows": rounds}
    output = root / "qa-lab" / "rebuild-v273" / "playstate-fix" / "qa-20x21-v273.json"; output.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"); print(json.dumps({"status": payload["status"], "rounds": 20, "total_reviews": 420, "complete_zip_crc": complete_status, "output": str(output)}, ensure_ascii=False)); return 0 if payload["status"] == "PASS" else 1
