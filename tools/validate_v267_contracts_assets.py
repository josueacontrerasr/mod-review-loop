...
def one(root: Path, song: str) -> dict[str, Any]:
    ...
    if not polymod.is_file(): errors.append("polymod_missing")
        ...
        if data.get("mod_version") != "2.6.7": errors.append(f"mod_version:{data.get('mod_version')}")
    ...
    if chart.get("generatedBy") != "Friday Night Funkin' - 0.8.6; V2.6.7 vocal RMS-VAD retimed holds and repetition-balanced player lanes d=0..3": errors.append("chart_generatedBy")
    ...
    for diff in DIFFICULTIES:
        ...
        if {int(note.get("d", -1)) for note in notes} != {0, 1, 2, 3}: errors.append(f"lane_coverage:{diff}")
        for i, note in enumerate(notes):
            ...
            if not isinstance(note.get("d"), int) or not 0 <= note.get("d", -1) <= 3: errors.append(f"player_lane:{diff}:{i}")
    if play.get("album") != f"esperon-{song}": errors.append(f"metadata_album:{play.get('album')}")
...
def main() -> int:
    ...
    payload = {"scope": "VSLICE_086_CONTRACTS_ASSETS_V267", "executed_at": datetime.now(timezone.utc).isoformat(), "target_version": "0.8.6", "mod_version": "2.6.7", "songs": len(rows), "passed": sum(row["status"] == "PASS" for row in rows), "status": "PASS" if all(row["status"] == "PASS" for row in rows) else "ERRORS_FOUND", "rows": rows}
    output = root / "qa-lab" / "rebuild-v267" / "playstate-fix" / "contracts-assets-v267.json"; output.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
