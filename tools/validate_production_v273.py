...
def onset_ms(item: dict[str, Any]) -> float:
    return float(item.get("audio_onset_ms", item.get("start_ms", 0.0)) or 0.0)
...
def direction_errors(notes: list[dict[str, Any]], syllables: list[dict[str, Any]], allowed_balanced: set[float]) -> list[dict[str, Any]]:
    ...
    for index, note in enumerate(notes):
        ...
        matches = [item for item in syllables if abs(onset_ms(item) - t) <= 1.5]
        if not matches:
            continue
...
def one(song: str) -> dict[str, Any]:
    ...
    evidence_dir = ROOT / "qa-lab/rebuild-v273/playstate-fix/vocal-sync-candidates-canonical" / song
    ...
    intervals = [(onset_ms(item) - 45.0, float(item.get("vocal_end_ms", 0.0)) + 45.0) for item in syll]
    spans = [(onset_ms(item) - 10.0, float(item.get("vocal_end_ms", 0.0)) + 20.0) for item in syll]
    starts = [onset_ms(item) for item in syll]
    errors: list[str] = []
    counts: dict[str, int] = {}
    outside = 0
    leaked = 0
    ...
    for diff in DIFFS:
        ...
        for reduction in reductions.get("reductions", {}).get(diff, []):
            ...
            if "repetition_balance" in reason or "repetition-balance" in reason or "spaced_directions" in reason or "safe_hard_subdivision" in reason or "dense_gt3" in reason or "reassigned_free_player_lane" in reason:
        ...
        for index, note in enumerate(notes):
            ...
            if length:
                ...
                covered = [item for item in syll if t - 5.0 <= onset_ms(item) <= end + 50.0]
...
def main() -> int:
    ...
    payload = {
        "scope": "PRODUCTION_VOCAL_SYLLABLE_ACCURATE_GATE_V273",
    ...
    output = ROOT / "qa-lab/rebuild-v273/playstate-fix/production-syllable-gate-v273.json"
