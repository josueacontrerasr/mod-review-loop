#!/usr/bin/env python3
"""Genera candidatos vocal-only refinados V2.6.1 a partir de Voices-*.ogg."""
...
def make_chart(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    for difficulty in ("easy", "normal", "hard"):
        notes[difficulty] = [{"t": round(float(time_ms), 3), "d": index % 4, "_source": "voice", "_voice_event_id": f"v261-{difficulty}-{index + 1:05d}"} for index, time_ms in enumerate(analysis["onsets"][difficulty])]
    return {"version": "2.0.0", "scrollSpeed": {"easy": 0.9, "normal": 1.0, "hard": 1.12}, "events": [], "notes": notes, "candidateOnly": True, "generatedBy": "Vocal-only V2.6.1 backtracked RMS refinement", "sourcePolicy": "NO_INSTRUMENTAL_NOTES"}
...
def analyze_one(root: Path, song: str) -> dict[str, Any]:
    ...
    try:
        ...
        output = root / "qa-lab" / "rebuild-v260" / "playstate-fix" / "vocal-only-v261" / song
...
def main() -> int:
    ...
    payload = {"scope": "VOCAL_ONLY_REFINED_CANDIDATES_V261", "executed_at": datetime.now(timezone.utc).isoformat(), "target_version": "0.8.6", "mod_version": "2.6.1", "songs": len(rows), "passed": sum(row["status"] == "PASS" for row in rows), "status": "PASS_CANDIDATES_ISOLATED" if all(row["status"] == "PASS" for row in rows) else "ERRORS_FOUND", "rows": rows, "instrumental_used_for_generation": False, "promotion": "BLOCKED_UNTIL_ALL_GATES_PASS"}
    output = root / "qa-lab" / "rebuild-v260" / "playstate-fix" / "vocal-only-v261" / "candidate-summary-v261.json"
