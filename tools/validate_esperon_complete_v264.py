    ...
    with zipfile.ZipFile(path) as archive:
        ...
        for root in roots:
            ...
            if polymod not in names: errors.append(f"polymod_missing:{root}")
                ...
                if meta.get('mod_version') != "2.6.4": errors.append(f"mod_version:{root}:{meta.get('mod_version')}")
            ...
            if chart in names:
                ...
                if data.get("generatedBy") != "Friday Night Funkin' - 0.8.6; V2.6.4 syllable-aligned vocal chart player lanes": errors.append(f"generatedBy:{root}")
...
def main() -> int:
    ...
    expected = {"Esperon-Completo.zip"} | {f"Mod-{'-'.join(word.capitalize() for word in song.split('-'))}-V2.6.4.zip" for song in SONGS}
    errors: list[str] = []
    ...
    payload = {"scope": "ESPERON_COMPLETE_ZIP_GATE_V264", "executed_at": datetime.now(timezone.utc).isoformat(), "mod_version": "2.6.4", "individual_zips": len(individual), "individual_passed": sum(row["status"] == "PASS" for row in individual), "complete": complete, "delivery_zip_count": len(zips), "status": "PASS" if not errors and len(individual) == 21 and all(row["status"] == "PASS" for row in individual) else "ERRORS_FOUND", "errors": errors, "individual": sorted(individual, key=lambda row: row["zip"])}
    output = root / "qa-lab" / "rebuild-v264" / "playstate-fix" / "zip-gate-v264.json"; output.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
