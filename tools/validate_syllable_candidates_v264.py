#!/usr/bin/env python3
"""Validate isolated V2.6.4 syllable-aligned candidates."""
from __future__ import annotations
import json
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
CAND = ROOT / 'qa-lab/rebuild-v264/playstate-fix/syllable-candidates-small'
...
def main():
    ...
    out={'scope':'V264_SYLLABLE_CANDIDATE_VALIDATION','status':'PASS' if not errors and len(rows)==21 else 'FAIL','songs':len(rows),'errors':errors,'rows':rows,'rules':{'notes_inside_vocal_alignment':True,'holds_do_not_cross_next_syllable':True,'lanes':'4..7 player strumline','difficulties':list(DIFFS)}}
    path=candidate_dir.parent/(candidate_dir.name+'-validation-v264.json')
