...
def load_one(root: Path, song: str, sim_root: Path) -> dict[str, Any]:
    ...
    elif not any(song in (read_json(path, errors, "level").get("songs", []) or []) for path in level_paths):
...
def main() -> int:
    ...
    payload = {
        "scope": "HEADLESS_MOBILE_VFS_V260",
    ...
    output = root / "qa-lab" / "rebuild-v260" / "mobile-loader-v260.json"
