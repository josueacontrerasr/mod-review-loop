...
def one(root: Path, song: str) -> dict[str, Any]:
    ...
    activity_path = root / "qa-lab" / "rebuild-v262" / "playstate-fix" / "vocal-only-v262" / song / "voice-activity.json"
...
def main() -> int:
    ...
    payload = {"scope": "PRODUCTION_VOCAL_ONLY_RUNTIME_GATE_V262", "executed_at": datetime.now(timezone.utc).isoformat(), "target_version": "0.8.6", "mod_version": "2.6.2", "songs": len(rows), "passed": sum(row["status"] == "PASS" for row in rows), "status": "PASS" if all(row["status"] == "PASS" for row in rows) else "ERRORS_FOUND", "total_notes_outside_vocal_segments": sum(row["outside_vocal_segments"] for row in rows), "total_candidate_metadata_leaked": sum(row["candidate_metadata_leaked"] for row in rows), "total_bad_player_lanes": sum(row["bad_player_lanes"] for row in rows), "rows": rows}
    output = root / "qa-lab" / "rebuild-v262" / "playstate-fix" / "production-vocal-gate-v262.json"
