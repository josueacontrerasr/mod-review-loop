...
SOURCE = ROOT / "qa-lab/rebuild-v272/playstate-fix/vocal-sync-candidates"
CANON = ROOT / "qa-lab/rebuild-v272/playstate-fix/vocal-sync-candidates-canonical"
BACKUP = ROOT / "qa-lab/rebuild-v272/playstate-fix/production-before-v272"
...
def main() -> int:
    ...
    for mod in sorted((ROOT / "mods").glob("esperon-dano-*")):
        ...
        backup_path = BACKUP / f"{song}-chart-before-v272.json"
    ...
    report = {
        "scope": "V272_SYLLABLE_ACCURATE_VOCAL_CHART_PROMOTION",
        "status": "PASS" if len(rows) == 21 else "FAIL",
        "songs": len(rows),
        "created_at": datetime.now(timezone.utc).isoformat(),
        "source": "V2.7.2 syllable-aligned candidates from verified vocal stems; no 500/1000 ms deletion; holds from measured sustained vocal intervals",
        "generatedBy": EXPECTED_GENERATED,
        "backup": str(BACKUP.relative_to(ROOT)),
        "rows": rows,
    }
    output = ROOT / "qa-lab/rebuild-v272/playstate-fix/vocal-sync-promotion-v272.json"
