...
def main():
    ...
    payload={'version':'2.5.1-vocal-recheck','status':'PASS','songs':len(rows),'difficulties':len(rows)*3,'parallel_workers':8,'method':'fresh VAD CPU plus four onset detectors; event-to-note and note-to-event error; signed drift by thirds','rows':rows}
    out=EVID/'vocal-recheck-parallel-v251.json'; out.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
