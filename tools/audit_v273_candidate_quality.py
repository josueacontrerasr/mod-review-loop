...
def attack_time(item):
    value = item.get('audio_onset_ms', item.get('start_ms', 0.0))
...
fal_examples = []
for song in SONGS:
    align = load(ROOT / 'qa-lab/rebuild-v273/playstate-fix/vocal-sync-candidates' / song / 'syllable-alignment.json')
    v273 = load(ROOT / 'qa-lab/rebuild-v273/playstate-fix/vocal-sync-candidates' / song / 'candidate-chart.json')
    v271 = load(ROOT / 'qa-lab/rebuild-v271/playstate-fix/density-candidates' / song / 'candidate-chart.json')
    syllables = sorted(align.get('syllables', []), key=attack_time)
    normal272 = v273.get('notes', {}).get('normal', [])
    ...
    for i, item in enumerate(syllables):
        ...
        if any(token in text for token in ('fal', 'tan')):
            fal_examples.append({'song': song, 'index': i, 'text': text, 'audio_onset_ms': item.get('audio_onset_ms'), 'normal273': note_near(normal272, attack_time(item)), 'normal271': note_near(normal271, attack_time(item))})
    lane_counts = Counter(int(n.get('d', -1)) for n in normal272)
    rows.append({
        'song': song,
        'syllables': len(syllables),
        'v271_normal_notes': len(normal271),
        'v273_easy_notes': len(v273.get('notes', {}).get('easy', [])),
        'v273_normal_notes': len(normal272),
        'v273_hard_notes': len(v273.get('notes', {}).get('hard', [])),
        ...
        'hard_hold_count': sum(1 for n in v273.get('notes', {}).get('hard', []) if float(n.get('l', 0.0) or 0.0) > 0),
...
summary = {
    ...
    'totals': {
        ...
        'v273_easy_notes': sum(r['v273_easy_notes'] for r in rows),
        'v273_normal_notes': sum(r['v273_normal_notes'] for r in rows),
        'v273_hard_notes': sum(r['v273_hard_notes'] for r in rows),
