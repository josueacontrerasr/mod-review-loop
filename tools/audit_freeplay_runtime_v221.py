#!/usr/bin/env python3
"""Deep static audit for the V2.2.1 runtime crash and visual contracts."""
...
def inspect_mod(mod: Path) -> dict:
    ...
    char_reports = [inspect_character(mod, chars.get("player", ""), errors), inspect_character(mod, chars.get("opponent", ""), errors)]
    stage_report = inspect_stage(mod, play.get("stage", ""), errors)
    ...
    return {"mod": mod.name, "song": song, "status": "PASS" if not errors else "ERROR", "errors": errors, "warnings": warnings, "album": album_report, "characters": char_reports, "stage": stage_report, "story_levels": level_links}
...
def main() -> int:
    ...
    payload = {"audit": "FREEPLAY_RUNTIME_V221", "target": "FNF Mobile V-Slice 0.8.6", "mods": len(reports), "passed": sum(r["status"] == "PASS" for r in reports), "errors": sum(len(r["errors"]) for r in reports), "status": "PASS" if len(reports) == 20 and all(r["status"] == "PASS" for r in reports) else "ERRORS_FOUND", "reports": reports}
    out = root / "qa-lab/rebuild-v221/freeplay-runtime-audit-v221.json"; out.parent.mkdir(parents=True, exist_ok=True); out.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n")
