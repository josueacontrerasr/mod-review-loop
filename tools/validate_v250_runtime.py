...
def one(root: Path, song: str) -> dict:
    ...
    if not manifest_path.is_file():
        ...
        if manifest.get("mod_version") != "2.5.0":
    ...
    for difficulty in ("easy", "normal", "hard"):
        ...
        if any(t < 0 or d < 0 or d > 3 for t, d in keys):
    ...
    if not style:
        ...
        if style.get("version") != "1.0.0":
            issues.append("note_style_version")
        if style.get("fallback") != "funkin":
            issues.append("note_style_fallback")
    ...
    if not album:
        ...
        for key in ("albumArtAsset", "albumTitleAsset"):
            ...
            if isinstance(asset, str) and not asset.startswith("freeplay/albumRoll/"):
                issues.append(f"{key}_path_contract")
    ...
    if not isinstance(play.get("characters", {}).get("playerVocals"), list) or not play.get("characters", {}).get("playerVocals"):
        issues.append("player_vocals_missing")
...
def main() -> int:
    ...
    payload = {"status": "PASS" if all(row["status"] == "PASS" for row in rows) else "ERRORS_FOUND", "version": "2.5.0", "songs": len(rows), "passed": sum(row["status"] == "PASS" for row in rows), "rows": rows, "runtime_limit": "Static contract/resolution check; native mobile playtest still required."}
    output = root / "qa-lab" / "rebuild-v250" / "runtime-contract-v250.json"
