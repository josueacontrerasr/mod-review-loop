...
def resolve_one(root: Path, song: str, variation: str = "default") -> dict[str, Any]:
    ...
    for difficulty in DIFFICULTIES:
        if difficulty not in notes: errors.append(f"difficulty_missing:{difficulty}")
            ...
            if {int(note.get("d", -1)) for note in notes[difficulty]} != {4, 5, 6, 7}: errors.append(f"lane_coverage:{difficulty}")
            for index, note in enumerate(notes[difficulty]):
                ...
                if "d" in note and (not isinstance(note["d"], int) or note["d"] < 4 or note["d"] > 7): errors.append(f"note_d_type_or_domain:{difficulty}[{index}]")
...
def main() -> int:
    ...
    payload = {
        "scope": "PLAYSTATE_VARIATION_RESOLVER_V265",
    ...
    output = root / "qa-lab" / "rebuild-v265" / "playstate-fix" / "playstate-resolver-production-v265.json"
