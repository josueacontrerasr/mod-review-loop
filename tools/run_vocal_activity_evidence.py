...
def one(root:Path,song:str)->dict:
    stem=root/f'sync-candidates/vocal-stems/{song}/vocals.wav'; audio=root/f'mods/esperon-dano-{song}/songs/{song}/Inst.ogg'; out=root/f'sync-candidates/vocal-activity/{song}.json'; out.parent.mkdir(parents=True,exist_ok=True)
    cmd=[sys.executable,str(root/'tools/analyze_vocal_activity.py'),str(stem),str(audio),'--output',str(out)]
    r=subprocess.run(cmd,capture_output=True,text=True)
    return {'song':song,'returncode':r.returncode,'stdout':r.stdout.strip(),'stderr':r.stderr.strip(),'output':str(out)}
def main():
    root=Path(sys.argv[1] if len(sys.argv)>1 else '.').resolve(); results=[]
    with ProcessPoolExecutor(max_workers=min(2,len(SONGS),os.cpu_count() or 2)) as pool:
    ...
    results.sort(key=lambda x:x['song']); payload={'status':'PASS' if all(x['returncode']==0 for x in results) else 'ERRORS_FOUND','songs':len(results),'results':results,'evidence_only':True}; out=root/'qa-lab/rebuild-v221/vocal-activity-summary.json'; out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n'); print(json.dumps({'status':payload['status'],'songs':len(results),'output':str(out)},ensure_ascii=False)); return 0 if payload['status']=='PASS' else 1
