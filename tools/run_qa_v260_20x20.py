...
def main() -> int:
    ...
    payload = {"status": "PASS" if all(item["status"] == "PASS" for item in rounds) else "ERRORS_FOUND", "version": "2.6.0-audit-v251", "rounds": 20, "mods_per_round": 20, "total_reviews": 400, "parallel_workers": 8, "rows": rounds, "scope": "Each round scans every source file and individual V2.5.1 ZIP CRC during V2.6.0 audit; collection is validated separately."}
    output = root / "qa-lab" / "rebuild-v250" / "qa-20x20-v260.json"
