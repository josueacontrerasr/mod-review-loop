...
DEFAULT_DIR = ROOT / "qa-lab/rebuild-v272/playstate-fix/vocal-sync-candidates"
...
def nearest_syllable(t: float, syllables: list[dict[str, Any]], tolerance: float = 15.0) -> dict[str, Any] | None:
    ...
    return item if abs(float(item.get("start_ms", 0.0)) - t) <= tolerance else None
...
def validate_song(song_dir: Path) -> tuple[list[tuple[Any, ...]], dict[str, Any]]:
    ...
    for difficulty in DIFFS:
        ...
        for index, note in enumerate(notes):
            ...
            if hold > 0.0:
                ...
                if syllable is None:
                    ...
                    start = float(syllable.get("start_ms", t))
        ...
        # There is intentionally no dense_500 failure in V2.7.2: 2–3 Spanish syllables
        # in 500 ms are valid separate attacks. The metric remains observable in reports.
        # V2.7.2 reports dense windows but does not delete valid vocal attacks.
...
def main() -> int:
    ...
    for song_dir in sorted(path for path in candidate_dir.iterdir() if path.is_dir()):
        ...
        rows.append(row)
    result = {
        "scope": "V272_SYLLABLE_ACCURATE_VOCAL_CANDIDATE_VALIDATION",
    ...
    output = candidate_dir / "batch-validation-v272.json"
