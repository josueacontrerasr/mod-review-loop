    ...
    with zipfile.ZipFile(path) as archive:
        ...
        for root in roots:
            ...
            if polymod not in names: errors.append(f"polymod_missing:{root}")
                ...
                if meta.get("mod_version") != "2.6.1": errors.append(f"mod_version:{root}:{meta.get('mod_version')}")
            ...
            if chart in names:
                ...
                if data.get("generatedBy") != "Friday Night Funkin' - 0.8.6": errors.append(f"generatedBy:{root}")
...
def main() -> int:
    ...
    expected = {"Esperon-Completo.zip"} | {f"Mod-{'-'.join(word.capitalize() for word in song.split('-'))}-V2.6.1.zip" for song in SONGS}
    errors: list[str] = []
    ...
    complete = inspect(delivery / "Esperon-Completo.zip", 20) if (delivery / "Esperon-Completo.zip").is_file() else {"status": "ERRORS_FOUND", "errors": ["complete_missing"]}
    if complete.get("status") != "PASS": errors.extend(complete.get("errors", []))
    payload = {"scope": "ESPERON_COMPLETE_ZIP_GATE_V261", "executed_at": datetime.now(timezone.utc).isoformat(), "mod_version": "2.6.1", "individual_zips": len(individual), "individual_passed": sum(row["status"] == "PASS" for row in individual), "complete": complete, "delivery_zip_count": len(zips), "status": "PASS" if not errors and len(individual) == 20 and all(row["status"] == "PASS" for row in individual) else "ERRORS_FOUND", "errors": errors, "individual": sorted(individual, key=lambda row: row["zip"])}
    output = root / "qa-lab" / "rebuild-v260" / "playstate-fix" / "zip-gate-v261.json"; output.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
