...
def one(root: Path, song: str) -> dict[str, Any]:
    ...
    if not polymod.is_file(): errors.append("polymod_missing")
        ...
        if data.get("mod_version") != "2.6.1": errors.append(f"mod_version:{data.get('mod_version')}")
...
def main() -> int:
    ...
    payload = {"scope": "VSLICE_086_CONTRACTS_ASSETS_V261", "executed_at": datetime.now(timezone.utc).isoformat(), "target_version": "0.8.6", "mod_version": "2.6.1", "songs": len(rows), "passed": sum(row["status"] == "PASS" for row in rows), "status": "PASS" if all(row["status"] == "PASS" for row in rows) else "ERRORS_FOUND", "rows": rows}
    output = root / "qa-lab" / "rebuild-v260" / "playstate-fix" / "contracts-assets-v261.json"; output.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
