...
def one(root: Path, song: str) -> dict:
    ...
    if not manifest_path.is_file():
        ...
        if manifest.get("mod_version") != "2.5.1":
...
def main() -> int:
    ...
    payload = {"status": "PASS" if all(row["status"] == "PASS" for row in rows) else "ERRORS_FOUND", "version": "2.5.1", "songs": len(rows), "passed": sum(row["status"] == "PASS" for row in rows), "rows": rows, "runtime_limit": "Static contract/resolution check; native mobile playtest still required."}
    output = root / "qa-lab" / "rebuild-v250" / "runtime-contract-v251.json"
