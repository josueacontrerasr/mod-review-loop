...
def load_one(root: Path, song: str, sim_root: Path) -> dict[str, Any]:
    ...
    for difficulty in ("easy", "normal", "hard"):
        ...
        for index, note in enumerate(entries):
            ...
            lane = int(note["d"])
            if timestamp < previous or timestamp < 0 or lane not in (0, 1, 2, 3):
                errors.append({"level": "ERROR", "code": "NOTE_CONTRACT", "path": str(song_dir), "detail": f"{difficulty}[{index}]"})
                break
            previous = timestamp
        if {int(item.get("d", -1)) for item in entries} != {0, 1, 2, 3}:
            errors.append({"level": "ERROR", "code": "LANE_COVERAGE", "path": str(song_dir), "detail": f"{difficulty}: expected d=0,1,2,3"})
...
def main() -> int:
    ...
    payload = {
        "scope": "HEADLESS_MOBILE_VFS_V273",
    ...
    output = root / "qa-lab" / "rebuild-v273" / "mobile-loader-v273.json"
