...
def main() -> int:
    ...
    manifest = {"scope": "ESPERON_COMPLETE_PACKAGE_V261", "executed_at": datetime.now(timezone.utc).isoformat(), "mod_version": VERSION, "songs": len(SONGS), "individual_zips": len(rows), "complete_zip": complete, "individual": rows, "delivery_entries": sorted(path.name for path in delivery.iterdir()), "status": "PASS" if len(rows) == 20 and complete["folders"] == [mod.name for mod in mods] else "ERRORS_FOUND", "policy": "Delivery contains only final ZIPs; runtime ZIPs exclude reports and text artifacts."}
    output = root / "qa-lab" / "rebuild-v260" / "playstate-fix" / "package-manifest-v261.json"
